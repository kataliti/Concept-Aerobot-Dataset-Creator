...
<div class="controls"><button id="play">▶ / ⏸ Пробел</button><button id="m1">−1 кадр</button><button id="p1">+1 кадр</button><button id="m10">−10</button><button id="p10">+10</button><button id="syncRight">Синхронизировать правую камеру</button><button id="syncLeft">Синхронизировать левую камеру</button></div>
...
function timestampAtVideoTime(v,timestamps){if(!D||!timestamps.length||!Number.isFinite(v.currentTime))return null;let x=Math.max(0,Math.min(timestamps.length-1,v.currentTime*D.video_fps)),i=Math.floor(x),j=Math.min(timestamps.length-1,i+1),u=x-i;return timestamps[i]+(timestamps[j]-timestamps[i])*u}
function videoTimeAtTimestamp(timestamps,t){if(!D||!timestamps.length||!Number.isFinite(t))return null;let j=near(timestamps,t);if(timestamps.length===1)return 0;let i=timestamps[j]<=t?j:Math.max(0,j-1),k=Math.min(timestamps.length-1,i+1);if(i===k)return i/D.video_fps;let span=timestamps[k]-timestamps[i],u=span>0?Math.max(0,Math.min(1,(t-timestamps[i])/span)):0;return (i+u)/D.video_fps}
function alignRightCamera(force=false){if(!D||!D.bottom_ts.length||f.readyState<1||b.readyState<1)return;let t=timestampAtVideoTime(f,D.forward_ts),target=videoTimeAtTimestamp(D.bottom_ts,t);if(target==null)return;let drift=target-b.currentTime,maxDrift=Math.max(0.012,0.5/D.video_fps);if(force||f.paused||Math.abs(drift)>maxDrift){safeSeek(b,target);b.playbackRate=f.playbackRate;return}b.playbackRate=Math.max(0.25,Math.min(4,f.playbackRate+drift*1.5))}
function syncRight(immediate=false){clearTimeout(bottomSeekTimer);if(immediate)alignRightCamera(true);else bottomSeekTimer=setTimeout(()=>alignRightCamera(true),70)}
function syncLeft(){if(!D||!D.forward_ts.length||!D.bottom_ts.length||b.readyState<1)return;let t=timestampAtVideoTime(b,D.bottom_ts),target=videoTimeAtTimestamp(D.forward_ts,t);if(target!=null)go(Math.round(target*D.video_fps))}
function maintainCameraSync(){if(!D||f.paused||f.seeking||f.readyState<1||b.readyState<1)return;alignRightCamera(false);if(b.paused&&!b.ended)b.play().catch(()=>{})}
function toggleSynchronizedPlayback(){if(f.paused){syncRight(true);b.playbackRate=f.playbackRate;f.play().catch(()=>{});b.play().catch(()=>{})}else{f.pause();b.pause();syncRight(true)}}
function pos(){if(requestedFrame==null&&!f.seeking)showPosition(fi());maintainCameraSync()}
function finishForwardSeek(){if(activeSeekFrame==null)return;clearTimeout(seekWatchdog);let completed=activeSeekFrame;activeSeekFrame=null;if(queuedFrame!=null&&queuedFrame!==completed){pumpForwardSeek();return}queuedFrame=null;requestedFrame=null;let i=fi();showPosition(i);syncRight()}
